  ...
  Slot.prototype.shuffleSlots = function () {
    const ceil = this.slots.size || 1;
    const floor = 0;
    let slotsTaken = new Set();
    let newOrder = 0;
    const resetContentPointers = this.resetContentPointers;
    for (let oSlotMap of this.slots) {
      ...
      slot.contentPointer = (resetContentPointers) ? "rear" : slot.contentPointer;
      //clear this loop's orders
      orders.clear();
    }
    slotsTaken.clear();
    this.resetContentPointers = false;
    return this.slots;
  };
  ...
  */
  ...
  SuperHeroMindMap.prototype.restart = function () {
    ...
    this.slot.resetContentPointers = true;
  ...
  SuperHeroMindMap.prototype.render = function () {
    ...
    //empty the Deck
    this.oContainer.empty();
    this.slotsActivatedAndPopulated = 0;
    ...
    for (const oSlotMap of this.slot.slots) {
      ...
      const asyncGetRenderReady = this.promise(
        (resolve, reject) => {
          ...
          slot.card = card;
          slot.uiHandle = unnecessaryContainerForSHODDYjQueryUI.css({
            display: "block",
            width: "100%"
          });
          const resolved = this.activateSlot(slot);
          if (!resolved) {
      ...
      asyncGetRenderReady.then(success => {
        ...
        this.oContainer.append(slot.card);
        return (this.slotsActivatedAndPopulated >= this.slot.slots.size) ? this.captureCanvasDimensions() : false;
      }, err => console.log(err));
    }
    return this;
  };
  SuperHeroMindMap.prototype.activateSlot = function (slot) {
  ...
  SuperHeroMindMap.prototype.populateSlot = function (slot) {
    ...
    return (!slot || slot == null) ? false : () => {
      ...
      slot.card.empty()
        ...
        .append(slot.uiHandle);
      return this.slotsActivatedAndPopulated++;
    };
  };
  SuperHeroMindMap.prototype.captureCanvasDimensions = function () {
  ...
  SuperHeroMindMap.prototype.rate = function () {
    ...
    const cardsAvailable = this.slot.slots.size - this.slotsMatched.size;
    ...
    const factor = (cardsAvailable >= this.slot.slots.size / 2) ? 1.5 : 1.25;
  ...
  SuperHeroMindMap.prototype.setHighScoringMatch = function (...highs) {
    ...
    const matchedSlot = this.slot.slots.get(match);
  ...
  SuperHeroMindMap.prototype.showStatistics = function () {
    ...
    matchedSuperheroes, unmatchedSuperheroes = null;
